...
class GRUAutoEncoder():
    def __init__(self, timesteps, input_dim):
        ...
        layer = GRU(64, activation='relu', return_sequences=True, kernel_regularizer=regularizers.l2(0.00))(input_layer)
        layer = GRU(32, activation='relu', return_sequences=False, kernel_regularizer=regularizers.l2(0.00))(layer)
        layer = RepeatVector(timesteps)(layer)
        layer = GRU(32, activation='relu', return_sequences=True, kernel_regularizer=regularizers.l2(0.00))(layer)
        layer = GRU(64, activation='relu', return_sequences=True, kernel_regularizer=regularizers.l2(0.00))(layer)
        output_layer = TimeDistributed(Dense(input_dim))(layer)

        self.gru_autoencoder = Model(inputs = input_layer, outputs = output_layer)
    
    def summary(self, ):
        self.gru_autoencoder.summary()
    ...
    def train(self, x, y, timesteps):
        ...
        self.gru_autoencoder.compile(optimizer='adam', loss='mean_squared_error')

        start_time = time.time()
        history = self.gru_autoencoder.fit(x, y, epochs=e, batch_size=b, validation_split=validation_split, verbose=2, shuffle=False)
        # Note: setting shuffle=False is important since we are using GRU
        end_time = time.time()
    ...
    def compute_threshold(self, x_thSet, timesteps):
        x_thSetPredictions = self.gru_autoencoder.predict(x_thSet)
    ...
    def predict(self, x_evaluation, timesteps):
        predictions = self.gru_autoencoder.predict(x_evaluation)
    ...
    def save_model(self, file_path):
        self.gru_autoencoder.save(file_path)
    ...
    def plot_reconstruction_error(self, x_evaluation, evaluationLabels, timesteps):
        ...
        for name, group in groups:
            ...
            ax.plot(group.index, group.reconstruction_error, marker = 'o' if int(name) == 0 else 'x', ms = 3, linestyle = '',
    ...
    def compute_mse(self, x_evaluation, predictions, timesteps):
        ...
        # To compute the mean squared error on the entire sequence use the following line instead of the previous two
