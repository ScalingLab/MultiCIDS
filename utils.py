...
def csv_header(file_path):
    try:
        with open(file_path, 'r') as csvfile:
            ...
            if any(is_numeric(item) for item in first_row):
...
def transform_data(training, val_test, has_header, names, features):
    if has_header:
        dfTrain = pd.read_csv(training, names=names, header=0, sep=',', index_col=False, dtype='unicode')
        ...
        try:
            with open('binary_outcomes.txt', 'r') as f:
                ...
                for i in range(window_size, len(lines)):
                    window = lines[i-window_size+1:i+1]
